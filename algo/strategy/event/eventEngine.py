...
class EventEngine:
    ...
    # ----------------------------------------------------------------------
    def start(self):
        self.__logger.info(EVENT_START)
        self.__active = True
        self.__thread.start()

    # ----------------------------------------------------------------------
    def stop(self):
        self.__logger.info(EVENT_STOP)
        self.__active = False
        self.__thread.join()

    # ----------------------------------------------------------------------
    def register(self, type_, handler):
        self.__logger.info(EVENT_REGISTER, type_)
        try:
    ...
    def unregister(self, type_, handler):
        self.__logger.info(EVENT_UNREGISTER, type_)
        try:
            handler_list_ = self.__handlers[type_]
    ...
    # ----------------------------------------------------------------------
    def put(self, event):
        self.__logger.debug(EVENT_IN_QUEUE, event.type_)
